...
class CodetoTamilUtil {
    private fun convertToBamini(unicodeStr: String): String {
    ...
    private fun convertToTam(unicodeStr: String): String {
        var unicodeStr = unicodeStr
        val TamilText = arrayOf(
            "þ", "þ£", "¬þ", "V", "r", "þ§", "þ¨", "ªþ", "«þ", "ªþ£",
        ...
        val TamilReplace = arrayOf(
            "க்ஷ", "க்ஷா", "க்ஷை", "க்ஷி", "க்ஷீ", "க்ஷு", "க்ஷூ", "க்ஷெ", "க்ஷே", "க்ஷொ",
        ...
        var count = 0
        while (count < TamilText.size) {
            unicodeStr = unicodeStr.replace(TamilText[count].toRegex(), TamilReplace[count])
            count++
        }
        return unicodeStr
    }
    ...
    companion object {
        const val BAMINI = 0
        const val TAM = 1
        fun convertToTamil(encodCode: Int, unicodeStr: String): String {
            var convertedStr = ""
            val tu = CodetoTamilUtil()
            if (encodCode == BAMINI) {
