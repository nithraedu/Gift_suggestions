...
package nithra.gift.suggestion.shop.birthday.marriage.crop_image;
...
public class CropImageActivity1 extends AppCompatActivity
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Bundle bundle = getIntent().getBundleExtra(CropImage1.CROP_IMAGE_EXTRA_BUNDLE);
        mCropImageUri = bundle.getParcelable(CropImage1.CROP_IMAGE_EXTRA_SOURCE);
        mOptions = bundle.getParcelable(CropImage1.CROP_IMAGE_EXTRA_OPTIONS);

        if (savedInstanceState == null) {
            if (mCropImageUri == null || mCropImageUri.equals(Uri.EMPTY)) {
                if (CropImage1.isExplicitCameraPermissionRequired(this)) {
                    // request permissions and handle the result in onRequestPermissionsResult()
                    requestPermissions(
                            new String[]{Manifest.permission.CAMERA},
                            CropImage1.CAMERA_CAPTURE_PERMISSIONS_REQUEST_CODE);
                } else {
                    CropImage1.startPickImageActivity(this);
                }
            } else if (CropImage1.isReadExternalStoragePermissionsRequired(this, mCropImageUri)) {
                // request permissions and handle the result in onRequestPermissionsResult()
                requestPermissions(
                        new String[]{Manifest.permission.READ_EXTERNAL_STORAGE},
                        CropImage1.PICK_IMAGE_PERMISSIONS_REQUEST_CODE);
            } else {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == CropImage1.PICK_IMAGE_CHOOSER_REQUEST_CODE) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                mCropImageUri = CropImage1.getPickImageResultUri(this, data);

                // For API >= 23 we need to check specifically that we have permissions to read external
                // storage.
                if (CropImage1.isReadExternalStoragePermissionsRequired(this, mCropImageUri)) {
                    // request permissions and handle the result in onRequestPermissionsResult()
                    requestPermissions(
                            new String[]{Manifest.permission.READ_EXTERNAL_STORAGE},
                            CropImage1.PICK_IMAGE_PERMISSIONS_REQUEST_CODE);
    ...
    public void onRequestPermissionsResult(
        ...
        if (requestCode == CropImage1.CAMERA_CAPTURE_PERMISSIONS_REQUEST_CODE) {
    ...
    protected void setResult(Uri uri, Exception error, int sampleSize) {
        int resultCode = error == null ? RESULT_OK : CropImage1.CROP_IMAGE_ACTIVITY_RESULT_ERROR_CODE;
    ...
    public void savefile(Uri sourceuri) {
        ...
        File mydir = getExternalFilesDir( "/Nithra/Gift Suggestions/");
        mydir.mkdirs();
        String fname = sharedPreference.getString(CropImageActivity1.this, "user_image_name") + ".jpg";
    ...
    protected Intent getResultIntent(Uri uri, Exception error, int sampleSize) {
        CropImage1.ActivityResult result =
                new CropImage1.ActivityResult(
        ...
        intent.putExtra(CropImage1.CROP_IMAGE_EXTRA_RESULT, result);
        return intent;
    }
