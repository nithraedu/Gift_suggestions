...
class ProductEdit : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        remove1!!.setOnClickListener(View.OnClickListener {
        ...
        remove2!!.setOnClickListener(View.OnClickListener {
        ...
        save!!.setOnClickListener(View.OnClickListener {
            ...
            println("---check_occ___ $gift_occasion")
            gift_gender = textView1!!.getTag().toString().trim { it <= ' ' }.replace("\r\n", "")
            println("---check_occ1___ $gift_gender")
        ...
        IVPreviewImage!!.setOnClickListener(View.OnClickListener { //choose_imge();
            val dialog: Dialog
            dialog = Dialog(
                this@ProductEdit,
        ...
        IVPreviewImage1!!.setOnClickListener(View.OnClickListener { //choose_imge1();
            val dialog: Dialog
            dialog = Dialog(
                this@ProductEdit,
                android.R.style.Theme_DeviceDefault_Light_Dialog_NoActionBar_MinWidth
        ...
        IVPreviewImage2!!.setOnClickListener(View.OnClickListener { //choose_imge2();
            val dialog: Dialog
            dialog = Dialog(
                this@ProductEdit,
                android.R.style.Theme_DeviceDefault_Light_Dialog_NoActionBar_MinWidth
    ...
    var mLauncher = registerForActivityResult<Intent, ActivityResult>(
        ...
        if (result.resultCode == RESULT_OK) {
            println("----------- new activity result$result")
            uri = result.data!!.data
            if (uri != null) {
                IVPreviewImage!!.setImageURI(uri)
                println("print_uri2== $uri")
                if (!image_change.contains(separated[0]!!)) {
                    image_change += separated[0] + ","
                }
                try {
        ...
        try {
            ...
            for (i in file_array.indices) {
                ...
                if (file_array[i] != null) {
                    ...
                    set_img++
                    println("print_image==" + Uri.fromFile(file_array[i]))
                }
            }
            println("check_size== " + file_array.size)
        } catch (e: Exception) {
            e.printStackTrace()
            println("printerror$e")
        }
        println("print map1 : $map1")
        println("print map2 : $map2")
        UploadAsync()
    }
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    if (response.body()!![0].status == "Success") {
                        ...
                        println("print_image== " + list_gift!![0].giftImage)
                        separated =
                    ...
                    for (i in occasion!!.indices) {
                        for (j in temp.indices) {
                            if (occasion!![i].id == temp[j]) {
                                println("--cat id select: " + occasion!![i].id)
                                println("--cat name select: " + occasion!![i].category)
                                selectedLanguage[i] = true
                                cat[i] = occasion!![i].category
                                cat_id[i] = occasion!![i].id
                                langList.add(i)
            ...
            override fun handleMessage(msg: Message) {
                val runnable = Runnable {
                    ...
                    if (applicationContext != null) {
                        println("====msg result : " + msg.obj.toString())
                        if (progressDialog.isShowing) {
                            progressDialog.dismiss()
                        }
                        if (msg.obj != null && msg.obj.toString().length != 0) {
                            println("result : " + msg.obj.toString())
                            ...
                            try {
                                ...
                                println("---output : " + jsonObject.getString("name"))
                    ...
                    try {
                        ...
                        try {
                            for ((key, value) in map2) {
                                ...
                                var file: File? = null
                                println("===error1 $value")
                                println("===error2 $file_name")
                        ...
                        try {
                            ...
                            println("===error4 " + e.message)
                        }
                        try {
                            println("===error5 $sb")
                        } catch (e: NullPointerException) {
                            e.printStackTrace()
                            println("===error6 " + e.message)
                        }
                        response = sb.toString()
                        println("result from server : $response")
                    } catch (e: IOException) {
                        println("===result error1 : " + e.message)
                        e.printStackTrace()
                    }
                } catch (e: Exception) {
